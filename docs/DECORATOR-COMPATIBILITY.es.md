...
| ------- | ----------------------- | ------- | ----- |
| `tsc` | TC39 Stage 3 | ✅ | Código fuente de la librería |
| `tsc` | Legacy `experimentalDecorators` | ✅ | `examples/compat/01-tsc-legacy/` |
| esbuild standalone | TC39 Stage 3 | ✅ | Descargas del playground |
| Rollup | TC39 Stage 3 | ✅ | Build de distribución browser |
...
| Vite (esbuild) | Legacy `experimentalDecorators` | ✅ | Requiere `useDefineForClassFields: false` |
| bun | TC39 Stage 3 | ✅ | `examples/compat/06-bun-tc39/` — requiere runtime bun |

Todos los setups anteriores están verificados por la suite de tests Playwright en `tests/browser/compat-decorator-setups.test.ts`.
