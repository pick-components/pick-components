...
| ------- | ----------------------- | ------- | ----- |
| `tsc` | TC39 Stage 3 | ✅ automatizado | Código fuente de la librería (tests CI unitarios e integación) |
| `tsc` | Legacy `experimentalDecorators` | ✅ automatizado | `examples/compat/01-tsc-legacy/` |
| esbuild standalone | TC39 Stage 3 | ✅ automatizado | Descargas del playground (build CI) |
| Rollup | TC39 Stage 3 | ✅ automatizado | Build de distribución browser (build CI) |
| webpack + ts-loader | TC39 Stage 3 | ✅ automatizado | `examples/compat/02-webpack-tc39/` |
| swc standalone | TC39 Stage 3 | ✅ automatizado | `examples/compat/03-swc-tc39/` |
| webpack + babel-loader | Legacy `experimentalDecorators` | ✅ automatizado | `examples/compat/04-babel-legacy/` |
| webpack + babel-loader | TC39 Stage 3 (2023-11) | ✅ automatizado | `examples/compat/05-babel-tc39/` |
...
| bun | TC39 Stage 3 | ✅ automatizado | `examples/compat/06-bun-tc39/` — requiere runtime bun |

Los setups etiquetados como **automatizado** están verificados por `tests/browser/compat-decorator-setups.test.ts` (ejemplos compat) o por el pipeline CI estándar (fuente de la librería, esbuild, Rollup). Los setups etiquetados como **verificado manualmente** han sido validados manualmente pero no tienen un ejemplo compat dedicado en este repositorio.
